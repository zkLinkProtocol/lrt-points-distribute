...
import { MagpieGraphQueryService, MagpieGraphTotalPoint } from './magpieGraphQuery.service';
...
export class MagpieService{
  ...
  private magpieData: MagpieData = {localTotalPoints: 0n, realTotalEigenpiePoints: 0n, realTotalEigenLayerPoints: 0n, items:[]};
  ...
  public async loadPointsData(){
    ...
    for (const item of localPoints) {
      const realEigenpiePoints = BigInt(item.points) * BigInt(realTotalPointsData.eigenpiePoints) / BigInt(localTotalPoints);
      const realEigenLayerPoints = BigInt(item.points) * BigInt(realTotalPointsData.eigenLayerPoints) / BigInt(localTotalPoints);
