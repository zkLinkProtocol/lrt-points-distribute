...
import {
  RsethData,
  RsethPointItemWithBalance,
  RsethPointItemWithoutBalance,
  RsethService,
} from "./rseth.service";
...
export class RsethController {
  ...
  public async getRsethPoints(
    ...
    return this.getReturnData(
      pointData.items,
      pointData.realTotalElPoints,
      pointData.realTotalKelpMiles,
    );
  }
  ...
  public async getAllRsethPoints(): Promise<Partial<RsethReturnDto>> {
    let pointData: RsethData;
    try {
  ...
  ): Promise<Partial<RsethReturnDto>> {
    let pointData: RsethData;
    try {
      pointData = await this.rsethService.getPointsData();
    } catch (err) {
