...
export class RsethService{
  ...
  private rsethData: RsethData = {localTotalPoints: BigInt(0), realTotalElPoints: 0, realTotalKelpMiles: 0, items:[]};
  ...
  public async loadPointsData(){
    ...
    let data: RsethPointItemWithBalance[] = [];
    ...
    for (const item of localPoints) {
      ...
      const elPointsPerToken = realTotalPointsData.get(bridgeToken)?.elPoints ?? 0;
      const kelpMilesPerToken = realTotalPointsData.get(bridgeToken)?.kelpMiles ?? 0;
  ...
  public getPointsData(address?: string): RsethData {
    let result: RsethData = cloneDeep(this.rsethData);
    if(address){
  ...
  public getPointsDataGroupByAddress(): RsethData{
    let result: RsethData = cloneDeep(this.rsethData);
    let data: Map<string, RsethPointItemWithoutBalance> = new Map;
